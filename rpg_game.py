...
class RPGGame:
    ...
    def create_menu(self):
        ...
        analysis_menu.add_command(label="Relatório ML", command=self.show_ml_report)
...
Dano Total Recebido: {stats['total_damage_taken']}"""
    ...
    def show_ml_report(self):
        from ml_system import BattleDifficultyClassifier, VictoryPredictor
        difficulty_classifier = BattleDifficultyClassifier()
        victory_predictor = VictoryPredictor()
        
        report = self.visualizer.generate_ml_report(difficulty_classifier, victory_predictor)
        
        report_window = tk.Toplevel(self.root)
        report_window.title("Relatório de Machine Learning")
    ...
    def on_battle_end_scene3(self, victory):
        # Registrar performance para ajuste de dificuldade
        damage_ratio = self.player.total_damage_dealt / max(1, self.player.total_damage_taken)
        self.difficulty_adjuster.record_battle(victory, 30, damage_ratio)  # Duração estimada
